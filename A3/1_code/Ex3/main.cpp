...
{
	glPushMatrix();
	glTranslatef(0+teapot[0], 0+teapot[1], 5+teapot[2]);
...
void key(unsigned char k, int x, int y)
{
	...
	case 'l': {//todo, hint:use the ARRAY that you defined, and notice the teapot can NOT be moved out the range of the table.
		teapot[0] += 0.1;
		if (teapot[0] > 1.5)	teapot[0] = 1.5;	// 防止移出桌面
		break;
			  }
	case 'j': {//todo
		teapot[0] -= 0.1;
	...
	case 'i': {//todo
		teapot[1] += 0.1;
		if (teapot[1] > 0.75)	teapot[1] = 0.75;
		break;
			  }
	case 'k': {//todo
		teapot[1] -= 0.1;
